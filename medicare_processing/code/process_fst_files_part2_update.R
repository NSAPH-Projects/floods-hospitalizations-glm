...
# location of files
dir.input = paste0('/n/dominici_nsaph_l3/Lab/projects/floods-hospitalizations-glm/medicare_processing/data_update/in_progress/')
...
for(year in years){
    ...
    library(plyr)
    dat.current = ddply(dat.current,.(zipcode_R,category_code,day,month,year),summarise,cases=sum(cases))
    dat.all=rbind(dat.all,dat.current)
}

# summarise one final time to get over entire multi-year period
dat_admissions_sum_total = ddply(dat.all,.(zipcode_R,category_code,day,month,year),summarise,cases=sum(cases))

#reorder
dat_admissions_sum_total = dat_admissions_sum_total[order(dat_admissions_sum_total$zipcode_R, dat_admissions_sum_total$category_code,dat_admissions_sum_total$year,dat_admissions_sum_total$month,dat_admissions_sum_total$day),]

# save processed admissions file
dir.output.local = paste0('/n/dominici_nsaph_l3/Lab/projects/floods-hospitalizations-glm/medicare_processing/data_update/in_progress/')
