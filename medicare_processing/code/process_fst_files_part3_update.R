...
file_denom = paste0('denom_by_year/confounder_exposure_merged_nodups_health_',year,'.fst')
...
for(i in seq((length(intervals)-1))){
    ...
    dat_denom_sum = ddply(dat_denom,.(year,zip),nrow)
    ...
    print(paste0('Number of total rows = ',dim(dat_denom_sum)[1]))
    print(paste0('Number of complete rows = ',dim(na.omit(dat_denom_sum))[1]))
...
dat_denom_sum_total = ddply(dat_denom_sum_total,.(year,zip),summarise,population=sum(population))
...
dir.output = paste0('/n/dominici_nsaph_l3/Lab/projects/floods-hospitalizations-glm/medicare_processing/data_update/in_progress_denom/')
...
dat_denom_sum_total_na = ddply(dat_denom_sum_total_na,.(year,zip),summarise,population=sum(population))
