...
# code.lookup.merged$category_desc = as.character(code.lookup.merged$category_desc)
cods = unique(code.lookup.merged$category_desc)

# set lag and cause of hospitalisation from seed value
# this code does include sub-causes in Level 1: 11,14,15,17,18 (don't know why RMP did not remove these) - did not remove bc I generate all expanded_grid_hospitalisations for CCS categories in dataset
...
# load cod data
dat.sample = readRDS(paste0(dir.input,'medicare_',gsub(" ", "_", cod.arg),'_rates_',years[1],'_',years[length(years)],'.rds'))

#change back to correct array (i.e. control rows are 0 all the way across)
dat.sample$exposed <- ifelse(dat.sample$control_indicator == 1 | dat.sample$control_indicator == 2, 0, dat.sample$exposed)
dat.sample$lag_wk1 <- ifelse(dat.sample$control_indicator == 1 | dat.sample$control_indicator == 2, 0, dat.sample$lag_wk1)
dat.sample$lag_wk2 <- ifelse(dat.sample$control_indicator == 1 | dat.sample$control_indicator == 2, 0, dat.sample$lag_wk2)
dat.sample$lag_wk3 <- ifelse(dat.sample$control_indicator == 1 | dat.sample$control_indicator == 2, 0, dat.sample$lag_wk3)
...
for (j in c(1:5)){
    val <- 0 
    for (k in seq(j, nrow(dat.sample), 5)){
        val <- val + dat2[k]
    }
    df[1,j] <- val 
} 
...
# create stratum with doy and fipscounty
...
#df is different in Level 3 vs Level 1 (the paper says df = 2), SA changed on 9/14 !!!
system.time({

    mod_dlm_unconstrained = gnm(cases ~  exposed + lag_wk1 + lag_wk2 + lag_wk3 + lag_wk4 + ns(year, df=2), 
                                data=dat.sample, offset=logpt, eliminate=factor(floodzip_id), family=quasipoisson)
...
#system.time({
    ...
    #mod_dlm_unconstrained_no_temp = gnm(cases ~ event_lag0 + event_lag1 + event_lag2 + event_lag3 + event_lag4 + event_lag5 + event_lag6 + event_lag7 +
        #ns(year, df=2) + dow , data=dat.sample.multiple, offset=logpop, eliminate=factor(stratum), family=quasipoisson)
...
# 
# output directory
dir.output.plots = paste0('/n/dominici_nsaph_l3/Lab/projects/floods-hospitalizations-glm/model_run/figures_UPDATE2/level3/unconstrained_dlm/')
dir.output.model.summary = paste0('/n/dominici_nsaph_l3/Lab/projects/floods-hospitalizations-glm/model_run/results_UPDATE2/level3/unconstrained_dlm/summary/')
dir.output.model.output = paste0('/n/dominici_nsaph_l3/Lab/projects/floods-hospitalizations-glm/model_run/results_UPDATE2/level3/unconstrained_dlm/model_output/')
...
#Unconstrained is spelt wrong 
pdf(paste0(dir.output.plots,'medicare_rr_values_model_summary_',cod.arg,'_',years[1],'_',years[length(years)],'.pdf'),paper='a4r',height=0,width=0)
print(
...
#forgot to delete the word 'update' in below 
...
# save w temperature model summary (#note: as of 12/17, does not include temeprature)
