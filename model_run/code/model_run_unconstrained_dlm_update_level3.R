...
ccs.names = read.csv('/n/dominici_nsaph_l3/Lab/projects/floods-hospitalizations-glm/medicare_processing/data/Single_Level_CCS_2015/dxlabel 2015.csv')
...
# this code does include sub-causes in Level 1: 11,14,15,17,18 (don't know why RMP did not remove these) - Fix !!!
# don't understand why RMP did not use the same file as he did for Level 1 - Fix !!!
...
dir.input = paste0('/n/dominici_nsaph_l3/Lab/projects/floods-hospitalizations-glm/medicare_processing/data/expanded_grid_hospitalisations/',years[1],'_',years[length(years)],'/')
...
system.time({
    ...
    mod_dlm_unconstrained = gnm(cases ~ event_lag0 + event_lag1 + event_lag2 + event_lag3 + event_lag4 + event_lag5 + event_lag6 + event_lag7 +
        cb1.temp + ns(year, df=2) + dow , data=dat.sample.multiple, offset=logpop, eliminate=factor(stratum), family=quasipoisson)
...
# output directory
dir.output.plots = paste0('/n/dominici_nsaph_l3/Lab/projects/floods-hospitalizations-glm/model_run/figures/unconstrained_dlm/')
dir.output.model.summary = paste0('/n/dominici_nsaph_l3/Lab/projects/floods-hospitalizations-glm/model_run/results/unconstrained_dlm/summary/')
dir.output.model.output = paste0('/n/dominici_nsaph_l3/Lab/projects/floods-hospitalizations-glm/model_run/results/unconstrained_dlm/model_output/')
