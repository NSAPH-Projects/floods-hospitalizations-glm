...
all_zipcodes = unique(zipcode_flood_data$zipcode)
...
add_date_info = function(dat, lag_chosen){
    ...
    dat = dat[,c('zipcode','year','month','day',col_name)]
...
#form: zipcode, year, month, day, eventlag0 = 1; zipcode, year, month, day + 1, eventlag1 = 1
for(lag in lags){
    assign(paste0('zipcode_flood_edit_',lag), add_date_info(zipcode_flood_data, lag))
    print(head(get(paste0('zipcode_flood_edit_',lag))))
...
zipcode_flood_edit_array = complete_grid

#merge the grid of zipcode/date combinations with the dataframes that have the lag columns 
# add multiple lags to array for all zipcodes in dataset
for(lag in lags){
    print(paste0('Matching lag ',lag))
    zipcode_flood_edit_array = merge(zipcode_flood_edit_array,get(paste0('zipcode_flood_edit_',lag)),by=c('zipcode','year','month','day'), all.x=TRUE)
}
zipcode_flood_edit_array[is.na(zipcode_flood_edit_array)] <- 0

# save as rds for analysis
saveRDS(zipcode_flood_edit_array, paste0(dir.output,'zipcode_flood_lag_array_',start_year,'_',end_year,'.rds'))