...
if(!file.exists(input.file)){
    ...
    for(cod.arg in causes_to_load){
        ...
        input.file.2 = # CCS level 2 input file 
...
nrow(dat)
    ...
    dat.temp <- #path to temperature data file
    dat$tmmx <- dat.temp$tmmx
    
    # merging humidity data with model matrix 
    dat.hum <- #path to humidity data file
...
# output directory
dir.output.plots = 
dir.output.model.summary = 
dir.output.model.output = 
