...
dir.output<- #set output directory 
load('zipcode_flood_subset_2000_2016_APR.Rdata') #load flood data 
...
max_duration <- max(flood_data_test$max)
...
for (i in 1:nrow(flood_data_test)){
  ...
  list_df[[i]] <- df1
  name <- paste0(flood_data_test$id[i],'_flood_df_',flood_data_test$zipcode[i])
...
for (i in 1:length(list_df)){
  list_df[[i]]$zipcode <- rep(flood_data_test$zipcode[i], flood_data_test$max[i] + 7*num_lagweeks)
  dates <- c()
  for (j in 0:(nrow(list_df[[i]]) - 1)){
    temp_date <- as.Date(flood_data_test$start[i], format="%Y-%m-%d") + j
  ...
  df3 <- data.frame(matrix(data = NA, nrow = flood_data_test$max[i] + 7*num_lagweeks, ncol = 7*num_lagweeks))
...
save(zipcode_flood_exp_lag_array_week, file = "zipcode_flood_exp_lag_array_week_2000_2016_APR.Rdata")
...
best.pairs1 <- matrix(0, nrow = nrow(flood_data_test), ncol = 2, byrow = TRUE)
best.pairs2 <- matrix(0, nrow = nrow(flood_data_test), ncol = 2, byrow = TRUE)
best.pairs3 <- matrix(0, nrow = nrow(flood_data_test), ncol = 2, byrow = TRUE)
...
save(control_dates_df_final, file = "control_dates_df_final_2000_2016.Rdata")
